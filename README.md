...
with the cross-calibrated data; however, the function will overwrite the
existing column if you set the option overwrite.col = T. The function
will also create an output directory that contains (1) trained Random
Forest models, (2) a spreadsheet with model evaluation metrics, and (3)
a multi-panel figure comparing sensors pre- and post-calibration.
