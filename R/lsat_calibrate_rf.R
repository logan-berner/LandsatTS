...
#' @param train.with.highlat.data Should the RF models be trained using an internal high-latitude dataset that sampled the Arctic and  Boreal biomes? 
#' @param add.predictors Vector of additional predictors to use in the Random Forest models. These should be time-invariant and match column names.
...
#' @param trim (True/False) If true, then the percent difference between satellites is determined for each sample and then the lowest 2.5 and highest 97.5 percentiles are trimmed.
...
#' The files include the (1) fitted random forest models as R objects, (2) evaluation data in a csv file, (3) summary of
...
lsat_calibrate_rf <- function(dt, band.or.si, doy.rng = 152:243, min.obs = 5, train.with.highlat.data = F, add.predictors = NULL, 
                              frac.train = 0.75, trim = T, overwrite.col = F, outfile.id=band.or.si, outdir){
  ...
  colnames(model.eval.df) <- c('band.or.si','sat','orig.bias','orig.bias.pcnt','rf.r2','rf.rmse','rf.n','xval.r2','xval.rmse','xval.n', 'xval.bias','xval.bias.pcnt')
  ...
  for (i in sats){
    ...
    full.fac <- data.table::data.table(expand.grid('sample.id' = unique(xcal.dt$sample.id), 'satellite' = unique(xcal.dt$satellite), 'doy' = doy.rng))
    ...
    # if using internal data to calibrate a spectral index, then derive the required spectral index
    if (train.with.highlat.data == T){
    ...
    if (trim == T){
      # rf.data.dt[, abs.dif.pcnt.rank := abs(dplyr::percent_rank(dplyr::desc(get(band.or.si) - get(paste('LANDSAT_7',band.or.si,sep='.')))))]
      # rf.data.dt <- rf.data.dt[abs.dif.pcnt.rank < 0.95]
      # rf.data.dt <- rf.data.dt[, abs.dif.pcnt.rank := NULL]
      rf.data.dt[, dif.pcnt.rank := dplyr::percent_rank(get(band.or.si) - get(paste('LANDSAT_7',band.or.si,sep='.')))]
