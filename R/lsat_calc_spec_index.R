...
lsat_calc_spec_index <- function(dt, si){
  ...
  dt <- data.table(dt)
  si <- tolower(si)
  if (si == 'evi'){data.table::dt[, evi := 2.5  * (nir - red) / (nir + 6 * red - 7.5 * blue + 1)]} # Huete et al. (2002) Remote Sensing of Environment
  if (si == 'evi2'){data.table::dt[, evi2 := 2.5  * (nir - red) / (nir + 2.5 * red + 1)]} # Jiang et al. (2008) Remote Sensing of Environment
  if (si == 'kndvi'){data.table::dt[, kndvi := tanh(((nir - red) / (nir + red))^2)]} # Camp-Valls et al. (2020) Science Advances
  if (si == 'msi'){data.table::dt[, msi := swir1 / nir]} # Rock et al. (1986) BioScience
  ...
  if (si == 'satvi'){data.table::dt[, satvi := 1.5 * (swir1 - red) / (swir1 + red + 0.5) - swir2/2]} # Marsett et al. (2006) Rangeland Eco. and Management
  dt
}