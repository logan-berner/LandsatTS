...
#' @param pixel_id_from The column name in pixel_coords_sf that specifies the pixel_id (defaults to "pixel_id" as generated by lsat_get_pixel_centres)
#' @param chunks_from Column mame in pixel_coords_sf to devide the exports into chunks. Over-rides chuncking by size (see max_chunk_size).
...
lsat_export_ts <- function(pixel_coords_sf,
  ...
  task_list <- pixel_coords_sf %>% split(., sf::st_drop_geometry(.)[,chunks_from]) %>%
    purrr::map(function(chunk){
      ...
      ee_chunk <- rgee::sf_as_ee(chunk[,c("geometry", pixel_id_from, chunks_from)])
